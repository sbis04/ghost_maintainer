...
/// `ghost_maintainer fix <issue_number>` — trigger bug fix for an issue.
class FixCommand extends Command<void> {
  @override
  final name = 'fix';

  @override
  final description = 'Investigate a bug and create a PR. Usage: ghost_maintainer fix <issue_number>';
  ...
  Future<void> run() async {
    if (argResults!.rest.isEmpty) {
      print('Usage: ghost_maintainer fix <issue_number>');
      exit(1);
    }
    ...
    await _triggerWorkflow(repo, token, issueNumber, 'bug');
  }

  Future<void> _triggerWorkflow(String repo, String token, String issueNumber, String type) async {
    print('Triggering bug fix for #$issueNumber on $repo...');
    ...
    if (response.statusCode == 204) {
      print('Bug fix triggered! Check GitHub Actions for progress:');
